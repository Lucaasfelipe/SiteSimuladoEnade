...
export class SimuladoComponent implements OnInit {
  ...
  constructor(public database: AngularFireDatabase, private modalService: NgbModal, public fire: AngularFireAuth) {
    ...
    /* LISTA DE PERGUNTAS
     this.perguntas = this.database.list('simulados/').snapshotChanges().map(arr => {
       let disc =  arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) )
       .filter(i => i.ano === this.valueYear)
       .map(i => i.questions[this.disciplina]);
       let discCompleto = [];
  ...
  delete(key: string) {
    console.log(key);
    const chaveAno = this.database.list('simulados/', ref => ref.orderByChild('ano').equalTo(this.valueYear)).snapshotChanges().map(arr => {
      return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }));
    }).take(1).forEach(item => {
      item;
    });
  ...
  enviarPerguntaUpload() {
    ...
    for (let x = 0; x < this.result.length; x++) {
      ...
      if (this.result[x].Pergunta === null ||
        this.result[x].Pergunta === undefined ||
        this.result[x].Pergunta === '') {

        this.validacao = 1;
      } else
        if (
          (this.result[x].resposta1 === null ||
            this.result[x].resposta1 === undefined ||
            this.result[x].resposta1 === '') ||
          (this.result[x].resposta2 === null ||
            this.result[x].resposta2 === undefined ||
            this.result[x].resposta2 === '') ||
          (this.result[x].resposta3 === null ||
            this.result[x].resposta3 === undefined ||
  ...
  getKey(dados) {
    return new Promise(resolve => {
      const retorno = this.database.list('simulados/').push(
        dados
      ).key;
