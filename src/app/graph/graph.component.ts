...
export class GraphComponent implements OnInit {
  ...
  constructor(
    ...
    // =============SELECT===================
    this.perguntas = this.database
      .list(this.tipoCaderno)
      .snapshotChanges()
      .map(arr => {
        return arr.map(snap =>
  ...
  consultar() {
    ...
    this.graficoLegend = this.database
      .list(this.tipoCaderno)
      .snapshotChanges()
      .map(arr => {
        return arr
          .map(snap => Object.assign(snap.payload.val(), { $key: snap.key }))
          .filter(i => Number(i.ano) === Number(this.year));
      });

    this.graficoLegend.forEach(item => {
      // console.log('item',item);

      const aux = Object.keys(item[0].questions).length;
      ...
      for (let x = 0; x < aux; x++) {
        ...
        my_obj.valor = JSON.parse(
          "[" + Object.keys(Object.values(item[0].questions)[x]).length + "]"
        );
    ...
    // ===========LINE===========
    this.graficoLegendLineSimulado = this.database
      .list("simulados/")
      .snapshotChanges()
      .map(arr => {
        return arr.map(snap =>
    ...
    this.graficoLegendLineProva = this.database
      .list("provas/")
      .snapshotChanges()
      .map(arr => {
        return arr.map(snap =>
    ...
    this.graficoLegendLineProva.forEach(item => {
      ...
      for (let x = 0; x < item.length; x++) {
        ...
        for (
          let y = 0;
          y < Object.values(Object.values(item[x].questions)).length;
          y++
        ) {
          aux += Object.keys(Object.values(Object.values(item[x].questions))[y])
            .length;
        }
  ...
  clickChart(event, content) {
    ...
    this.perguntasModal = this.database
      .list(this.tipoCaderno)
      .snapshotChanges()
      .map(arr => {
        return arr
          .map(snap => Object.assign(snap.payload.val(), { $key: snap.key }))
          .filter(i => Number(i.ano) === Number(this.year))
          .map(u => u.questions[this.nameDisciplinaModal]);
      });
