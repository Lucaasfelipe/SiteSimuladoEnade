...
export class ProvaComponent implements OnInit {
  ...
  constructor(public database: AngularFireDatabase, private modalService: NgbModal, public fire: AngularFireAuth) {
    ...
    /* LISTA DE PERGUNTAS
     this.perguntas = this.database.list('provas/').snapshotChanges().map(arr => {
       const disc =  arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) )
       .filter(i => i.ano === this.valueYear).map(i => i.questions[this.disciplina]);
       const discCompconsto = [];
       disc[0].forEach(function(element, index, array){
         const eachValue = element;
         eachValue['key'] = index;
         discCompconsto.push(eachValue);
         //console.log(element);
       })
       //console.log(disc[0]);

       return discCompconsto;
     });
   */

    this.aux = this.database.list('provas/').snapshotChanges().map(arr => {
      return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }))
        .filter(i => i.ano === this.valueYear);
    });
    this.aux.take(1).forEach(item => {
      ...
      if (item.length > 0) {
        this.getChaveAno(item[0]).then((chaveAno) => {
          ...
          this.perguntas = this.database.list(`provas/${chaveAno}/questions/${this.disciplina}`)
            .snapshotChanges().map(arr => {
              return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }));
            });
          this.valueChanges = this.database.list(`provas/${chaveAno}/questions/${this.disciplina}`).valueChanges();
        ...
        this.perguntas = this.database.list(`provas/8000/questions/${this.disciplina}`).snapshotChanges().map(arr => {
          return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }));
        });
        this.valueChanges = this.database.list(`provas/8000/questions/${this.disciplina}`).valueChanges();
        this.valueChanges.forEach(e => {
    ...
    this.aux.take(1).forEach(item => {
      ...
      if (item.length > 0) {
        this.getChaveAno(item[0]).then((chaveAno) => {
          ...
          this.perguntas = this.database.list(`provas/${chaveAno}/questions/${this.disciplina}`).snapshotChanges().map(arr => {
            return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }));
          });
          this.valueChanges = this.database.list(`provas/${chaveAno}/questions/${this.disciplina}`).valueChanges();
          this.valueChanges.forEach(e => {
  ...
  update() {
    ...
    this.database
      .list(`provas/${this.chaveAno}/questions/${this.disciplina}`)
  ...
  deconste(key: string) {
    // console.log(key);
    const chaveAno = this.database.list('provas/', ref => ref.orderByChild('ano')
      .equalTo(this.valueYear))
      .snapshotChanges()
      .map(arr => {
        return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }));
      }).take(1).forEach(item => {
    ...
    swal({
      ...
      if (result.value) {
        this.database.list(`provas/${this.chaveAno}/questions/${this.disciplina}`).remove(key);
        swal(
          'Excluído!',
          'A pergunta foi deconstada com sucesso!.',
          'success'
        );
      }
    });
  ...
  next(number: number) {
    if (this.pergunta !== undefined && this.currentTab === 0 && number !== -1 && this.pergunta !== '') {
    ...
    } else if (this.resposta1 !== undefined &&
      this.currentTab === 1 &&
      this.resposta1 !== '' &&
      this.justificativaresposta1 !== undefined &&
      this.justificativaresposta1 !== '' &&
      this.justificativaresposta1 !== null) {
      this.currentTab = this.currentTab + number;
    } else if (this.resposta2 !== undefined &&
      this.currentTab === 2 &&
      this.resposta2 !== '' &&
      this.justificativaresposta2 !== undefined &&
      this.justificativaresposta2 !== '' &&
      this.justificativaresposta2 !== null) {
      this.currentTab = this.currentTab + number;
    } else if (this.resposta3 !== undefined &&
      this.currentTab === 3 &&
      this.resposta3 !== '' &&
      this.justificativaresposta3 !== undefined &&
      this.justificativaresposta3 !== '' &&
      this.justificativaresposta3 !== null) {
      this.currentTab = this.currentTab + number;

    } else if (this.resposta4 !== undefined &&
      this.currentTab === 4 &&
      this.resposta4 !== '' &&
      ...
      */
      ...
      /*
         const ano = this.database.list('provas/').snapshotChanges().map(arr => {
          return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) )
        }).take(1).forEach(v => {
          v.filter(i => i.ano === this.valueYear);
          if(v[0].ano === this.valueYear){
            this.database.list('provas/' + this.valueYear + '/question/' + this.disciplina).snapshotChanges().map(arr => {
              return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) )
            }).take(1).forEach(v => {
              if(v.length === null || v.length === undefined){
               this.numeroQuestao = 1;
               this.database.object('provas/' + this.valueYear + '/question/' + this.disciplina +'/'+ this.numeroQuestao).set(
                 dados
               )
             }else{
      ...
      antes
        const chaveAno =  this.database.list('provas/').snapshotChanges().map(arr => {
          return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) ).filter(i => i.ano === this.valueYear)
        }).take(1).forEach(item =>{
          const questions = [];
          if(item.length > 0){
            ...
            this.database.object('provas/'+item[0].$key + '/questions/' + this.disciplina).set(questions);
          } else {
            const novo = {
              ano: this.valueYear,
              questions: {
            ...
            this.database.list('provas/').push(novo);
          }
        });
        */
      ...
      if (this.chaveAno === undefined || this.chaveAno === null) {
        this.database.list('provas/').push(
          dados
        );
      } else {
      ...
      /*
         const pergunta;
         pergunta = this.database.list('provas/').snapshotChanges().map(arr => {
           return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) )
         }).take(1).forEach(v => {
           if(v.length === null || v.length === undefined){
            this.numeroQuestao = 1;
    ...
    for (let x = 0; x < this.result.length; x++) {
      ...
      if (this.result[x].Pergunta === null || this.result[x].Pergunta === undefined || this.result[x].Pergunta === '') {
        ...
        if (
          ...
          if (this.result[x].respostaCorreta === null ||
            this.result[x].respostaCorreta === undefined ||
            this.result[x].respostaCorreta === '') {
            this.validacao = 1;
          } else
            if (
              (this.result[x].justificativaresposta1 === null ||
                this.result[x].justificativaresposta1 === undefined ||
                this.result[x].justificativaresposta1 === '') ||
              (this.result[x].justificativaresposta2 === null ||
                this.result[x].justificativaresposta2 === undefined ||
                this.result[x].justificativaresposta2 === '') ||
              (this.result[x].justificativaresposta3 === null ||
                this.result[x].justificativaresposta3 === undefined ||
              ...
              if (this.chaveAno === undefined || this.chaveAno === null) {
                ...
                if(this.validaDisciplina.length>0){
                 console.log('erro2')
                 console.log('validaDisciplina',this.validaDisciplina);

                this.setDisciplina(dados1).then(resolve =>{
              ...
              /*
            this.database.list('provas/' + this.chaveAno).push({
              pergunta: this.result[x].Pergunta,
              ano: this.valueYear,
  ...
  getKey(dados) {
    return new Promise(resolve => {
      const retorno = this.database.list('provas/').push(
        dados
      ).key;
  ...
  updateValidaDisiciplina() {
    return new Promise(resolve => {
      ...
      this.valueChanges = this.database.list(`provas/${this.chaveAno}/questions/${this.disciplina}`).valueChanges();
      this.valueChanges.forEach(e => {
