...
/*
   let ano = this.database.list('provas/').snapshotChanges().map(arr => {
    return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) )
  }).take(1).forEach(v => {
...
antes
  let chaveAno =  this.database.list('provas/').snapshotChanges().map(arr => {
    return arr.map(snap => Object.assign(snap.payload.val(), { $key: snap.key }) ).filter(i => i.ano == this.valueYear)
  }).take(1).forEach(item =>{
    let questions = []; 
    if(item.length > 0){
...
this.selectAno();
  ...
  getKey(dados){
    return new Promise(resolve => {
      let retorno = this.database.list('provas/').push(
            dados
          ).key;
